...
# 依赖：pip install qrcode[pil] pillow pyzbar reedsolo cryptography opencv-python moviepy
# 增强版：支持跨帧FEC（15-30%丢帧恢复）+ AES-256-GCM加密
...
def encrypt_payload_aes_gcm(data: bytes, passphrase: str, pubkey_pem: bytes) -> bytes:
    ...
    nonce = secrets.token_bytes(12)  # GCM模式推荐12字节nonce
    ...
    # 组装加密包：版本 + salt + nonce + 密钥长度 + 加密密钥 + 密文
    header = b"AES256GCM"  # 9字节版本标识
...
def create_cross_frame_fec(data_chunks: list[bytes], fec_ratio: float = 0.3) -> list[bytes]:
    """
    创建跨帧FEC编码，支持15-30%丢帧恢复
    使用Reed-Solomon编码，将数据块分组并添加冗余块
    """
...
def main():
    ...
    ap.add_argument('--fec-ratio', type=float, default=0.3, help='FEC冗余比例(0.15-0.35)')
    ...
    # 切片
    chunk_size = 800  # 减小块大小以适应QR码容量
    chunks = list(chunk_bytes(enc_payload, chunk_size))

    # 跨帧FEC编码
    if args.enable_fec:
        fec_ratio = max(0.15, min(0.35, args.fec_ratio))  # 限制在15-35%
    ...
    for i in range(len(fec_chunks)):
        is_fec_block = i >= len(chunks)  # 超出原始块数的为FEC冗余块
